...
export async function POST(
  ...
  try {
    ...
    if (nowUnix < campaign.startAtUnix) {
      return json({ error: "Campaign has not started yet" }, { status: 400 });
    }
    ...
    if (!isVerified) {
      ...
      try {
        ...
        if (refreshed && updatedVerified === false) {
          return json({ error: "X account must be verified to join this campaign" }, { status: 403 });
        }
    ...
    try {
      ...
      return json({ error: "Failed to verify token balance" }, { status: 503 });
    }
