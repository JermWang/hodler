...
export async function POST(req: Request, ctx: { params: { id: string; milestoneId: string } }) {
  ...
  try {
    ...
    const [balanceLamports0, nowUnix] = await Promise.all([
      ...
      if (Number.isFinite(ageSeconds) && ageSeconds > 120) {
        ...
        try {
          ...
          if (safe === "RPC error (blockhash expired)") {
            ...
            await deleteRewardMilestonePayoutClaim({ commitmentId: id, milestoneId });
            await auditLog("admin_reward_milestone_release_error", {
              commitmentId: id,
              milestoneId,
              error: safe,
              clearedClaim: true,
            ...
            return NextResponse.json(
              {
                error: safe,
                hint: "Temporary RPC issue while sending the payout transaction. Please retry the release.",
              },
              { status: 503 }
            );
          }

          await auditLog("admin_reward_milestone_release_error", { commitmentId: id, milestoneId, error: safe });
