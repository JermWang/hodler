...
import { updateHodlrEpochStatus } from "@/app/lib/hodlr/store";
...
async function handle(req: NextRequest) {
  try {
    ...
    const updated = await updateHodlrEpochStatus({ epochId, status: "claim_closed" });

    return NextResponse.json({ ok: true, epoch: updated, flags });
  } catch (e) {
    return NextResponse.json(
      { error: "HODLR claim close failed", details: e instanceof Error ? e.message : String(e) },
      { status: 500 }
    );
  }
}
