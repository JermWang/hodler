...
async function handle(req: NextRequest) {
  try {
    ...
    const result = await runHodlrDistributionDryRunShadow({ epochId: epochId || undefined });

    return NextResponse.json({ ...result, flags });
  } catch (e) {
    return NextResponse.json(
      { error: "HODLR distribution dry run failed", details: e instanceof Error ? e.message : String(e) },
      { status: 500 }
    );
  }
}
